...
def get_input(filename='aoc2016_day18.txt'):
    with open(filename) as f:
        for line in f:
            return [x for x in line]

def is_safe_tile(previous_row, col_no):
    traps = ('^^.', '.^^', '^..', '..^')
    return ''.join(previous_row[col_no - 1: col_no + 2]) not in traps

def count_safe_tiles(grid):
    safe_tiles = (Counter(row[1:-1])['.'] for row in grid) # do not include the extra safe left and right column added to make checking safe tiles simpler.
    return 'Number of safe tiles is {}'.format(sum(safe_tiles))

def append_rows(grid, rows=40, cols=100):
    for row_no in range(1, rows):
        print('calculating row: {}'.format(row_no))
        new_row = ['.'] + ['.' if is_safe_tile(grid[row_no - 1], col_no) else '^' for col_no in range(1, cols + 1)] + ['.'] # adding extra safe tile column at start and end for easier checking.
        grid.append(new_row)
    return grid

def q1():
    grid = [['.'] + get_input() + ['.']]
    grid = append_rows(grid, rows=40)
    print(len(grid), len(grid[1]))
    print(count_safe_tiles(grid))

def q2():
    grid = [['.'] + get_input() + ['.']]
    grid = append_rows(grid, rows=400000)
    print(len(grid), len(grid[1]))
    print(count_safe_tiles(grid))

def test():
    grid = [['.'] + [x for x in '.^^.^.^^^^'] + ['.'],]
    grid = append_rows(grid, cols=10, rows=10)
    print(count_safe_tiles(grid))
