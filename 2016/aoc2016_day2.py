...
def bathroom_security_1(input_strings):
    ...
    for line in input_strings:
        for move in line:
            new_position = [sum(x) for x in zip(current_position, moves[move])]
            if all([0 <= x <= 2 for x in new_position]):
                current_position = new_position
        answer.append(str(keypad[current_position[1]][current_position[0]]))

    return ''.join(answer)

def bathroom_security_2(input_strings):
    moves = {'U': (0, -1), 'D': (0, 1), 'L': (-1, 0), 'R': (1, 0)}
    ...
    for line in input_strings:
        for move in line:
            new_position = [sum(x) for x in zip(current_position, moves[move])]
            if all([0 <= x <= 4 for x in new_position]):
                if keypad[new_position[1]][new_position[0]] != 0:
                    current_position = new_position
        answer.append(str(keypad[current_position[1]][current_position[0]]))
...
def testing():
    ...
    Testing
    '''
    print(bathroom_security_1(['ULL', 'RRDDD', 'LURDL', 'UUUUD']), 1985)

    input_strings = [line.strip('\n') for line in open('aoc2016_day2.txt', 'r')]
    for item in input_strings:
        print(item)
    print(bathroom_security_1(input_strings), 24862)

    print(bathroom_security_2(['ULL', 'RRDDD', 'LURDL', 'UUUUD']), '5DB3')
    print(bathroom_security_2(input_strings), '...')
