...
def dragon_checksum(data, disk_space, print_statements=False):
    '''
    See dragon curve - https://www.youtube.com/watch?v=wCyC-K_PnRY

    Generate new data using below method until disk_space is full.

    new_data = data + '0' + reversed (i.e [::-1]) and flipped (i.e 0 = 1, 1 = 0)

    Using only data in disk_space, generate checksum as below:
    ...
    If the length of the checksum is even, repeat the process until you end up with a checksum with an odd length.
    '''
    while len(data) < disk_space:
        data = data + '0' + ''.join(('0' if int(x) else '1' for x in data[::-1]))
        if print_statements: print(len(data))

    data = data[:disk_space]

    while len(data) % 2 == 0:
        if print_statements: print(len(data))
        data = ['1' if data[idx] == data[idx + 1] else '0' for idx in range(0, len(data), 2)]
    return ''.join(str(x) for x in data)

print(dragon_checksum('110010110100', 12), '100')
print(dragon_checksum('10000', 20), '01100')
print(dragon_checksum('10001110011110000', 272), '10010101010011101') # my puzzle input
print(dragon_checksum('10001110011110000', 35651584, print_statements=True), '01100111101101111') # my puzzle input
